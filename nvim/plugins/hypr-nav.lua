...
--
-- NOTE: the `hypr-nav` binary is not installed on this machine. Its C source
-- still lives only on the `main` branch (.local/src/hypr-nav) and has not been
-- ported. Until it is built onto PATH, the bindings below still move between
-- vim splits; the hand-off at the edge is a silent no-op.
